...
{{-- Content --}}
@section ('content')
    <data-form action="{{ URL::route('bazar.orders.create') }}" :model="{{ $order }}">
        <template #default="form">
                ...
                <div v-if="form.fields.user.id" class="form-group mt-0">
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-group-append">
                            <button type="button" class="btn btn-secondary" @click.prevent="form.fields.user = { addresses: [] }">
                                {{ __('Remove') }}
                            </button>
                        </div>
                    </div>
            ...
            <card :title="__('Billing')" class="mb-5">
                <template #header>
                    <div class="form-group">
                        <select
                            ...
                            @input.prevent="event => form.fields.address = JSON.parse(event.target.value)"
                        >
                ...
                <div class="row">
                    <div class="col-12 col-sm-6">
                        <form-select name="address.country" :label="__('Country')" :options="{{ json_encode($countries) }}" v-model="form.fields.address.country"></form-select>
                    </div>
                    <div class="col-12 col-sm-6">
            ...
            <card :title="__('Shipping')" class="mb-5">
                <template #header>
                    <div class="form-group">
                        <select
                            class="custom-select custom-select-sm"
                            :disabled="! form.fields.user.addresses.length"
                            @input.prevent="event => form.fields.shipping.address = JSON.parse(event.target.value)"
                        >
                            <option :value="null" selected>--- {{ __('Address') }} ---</option>
                            <option v-for="(address, index) in form.fields.user.addresses" :key="index" :value="JSON.stringify(address)">
                                @{{ address.alias }}
                ...
                <div class="row">
                    <div class="col">
                        <form-select name="shipping.driver" :label="__('Driver')" :options="{{ json_encode($drivers) }}" v-model="form.fields.shipping.driver"></form-select>
                    </div>
                    <div class="col">
                        <form-input
                            name="shipping.cost"
                ...
                <div class="row">
                    <div class="col-12 col-sm-6">
                        <form-select name="shipping.address.country" :label="__('Country')" :options="{{ json_encode($countries) }}" v-model="form.fields.shipping.address.country"></form-select>
                    </div>
                    <div class="col-12 col-sm-6">
            ...
            <card :title="__('Products')">
                <form-autocomplete
                    name="products"
                    endpoint="{{ URL::route('bazar.products.index') }}"
                    multiple
        ...
        <template #aside="form">
            <card :title="__('Settings')" class="mb-5">
                ...
                ></form-input>
                <form-select name="currency" :label="__('Currency')" :options="{{ json_encode($currencies) }}" v-model="form.fields.currency"></form-select>
