<script>
    ...
    export default {
        ...
        methods: {
            ...
            copy(address, to) {
                if (to === 'billing') {
                    Object.assign(this.$refs.form.fields.address, address || {});
                } else if (to === 'shipping') {
                    Object.assign(this.$refs.form.fields.shipping.address, address || {});
                }
            }
        }
    }
        ...
        <data-form ref="form" :action="$page.action" :model="$page.order">
            <template #default="form">
                <card :title="__('User')" class="mb-5">
                    ...
                    <div v-if="form.fields.user" class="form-group mt-0">
                        <label>{{ __('User') }}</label>
                    ...
                    <template #header>
                        <div>
                            <div class="input-group input-group-sm">
                                <select class="custom-select" :disabled="! addresses.length" v-model="billing">
                                ...
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-outline-primary" :disabled="! billing" @click.prevent="copy(billing, 'billing')">
                                        {{ __('Copy') }}
                                    </button>
                                </div>
                            </div>
                ...
                <card :title="__('Shipping')" class="mb-5">
                    <template #header>
                        <div>
                            <div class="input-group input-group-sm">
                                <select class="custom-select" :disabled="! addresses.length" v-model="shipping">
                                    <option :value="null" disabled selected>--- {{ __('Address') }} ---</option>
                                    <option v-for="(address, index) in addresses" :key="index" :value="address">
                                        {{ address.alias }}
                                    </option>
                                ...
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-outline-primary" :disabled="! shipping" @click.prevent="copy(shipping, 'shipping')">
                                        {{ __('Copy') }}
                                    </button>
                                </div>
                            </div>
